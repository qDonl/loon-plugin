 ...
 *
 * 用法：在 Safari 访问 http://rednot.manage （需 Loon 处于运行状态）
 * Loon 拦截该请求并返回 HTML 管理界面，无需安装额外 App。
 *
 * 路由：
 *   GET /          → 黑名单管理页（HTML）
...
(function main() {
  ...
  if (url.endsWith("/remove")) {
    ...
    if (!userId) return jsonResponse({ success: false, error: "missing user_id" });
    ...
    return jsonResponse({ success: true, removed: before - newList.length, count: newList.length });
  }
...
function buildHTML(blacklist) {
  ...
  let rows = "";
  if (total === 0) {
    rows = `<div class="empty"><span class="eico">🚫</span>黑名单为空<br><small>长按笔记「不感兴趣」或详情页按钮可添加</small></div>`;
  } else {
    ...
    ORDER.forEach(src => {
      ...
      list.forEach((u, i) => {
        ...
        <div class="item${sep}">
          <div class="info">
            <div class="name">${escHtml(disp)}</div>
            ${name ? `<div class="uid">${escHtml(uid)}</div>` : ""}
          </div>
          <span class="date">${formatDate(u.added_at)}</span>
          <button class="del" onclick="del('${escHtml(uid)}','${escHtml(disp)}')">移除</button>
        </div>`;
      });
      rows += `</div>`;
    });
...
.topbar{position:sticky;top:0;z-index:9;background:rgba(242,242,247,.9);-webkit-backdrop-filter:blur(10px);backdrop-filter:blur(10px);padding:12px 16px;border-bottom:.5px solid rgba(0,0,0,.12);display:flex;align-items:baseline;gap:8px}
.topbar h1{font-size:17px;font-weight:600}
.topbar .cnt{font-size:14px;color:#8e8e93}
.sec-title{font-size:13px;color:#8e8e93;padding:20px 16px 8px;font-weight:500}
...
.badge{font-size:11px;padding:2px 6px;border-radius:8px;font-weight:500}
.date{font-size:12px;color:#c7c7cc;margin:0 10px;white-space:nowrap}
.del{color:#ff3b30;font-size:14px;font-weight:500;border:none;background:none;padding:4px 0 4px 8px;cursor:pointer;white-space:nowrap}
.empty{text-align:center;padding:64px 32px;color:#8e8e93;line-height:1.8}
...
.tip{margin:16px 16px 0;background:#fff;border-radius:12px;padding:14px 16px}
.tip h3{font-size:14px;font-weight:600;margin-bottom:8px;color:#1c1c1e}
.tip ul{font-size:14px;color:#444;line-height:2;padding-left:18px}
.tip code{font-family:monospace;font-size:12px;background:#f2f2f7;padding:1px 5px;border-radius:4px}
</style>
</head>
<body>
...
<div class="tip">
  ...
  <ul>
    ...
    <li><b>手动添加</b>：Loon → 插件 → 小红书黑名单 → 配置 → 输入用户ID</li>
    <li><b>用户ID</b>：小红书主页 URL 中 <code>/user/profile/</code> 后的字符串</li>
    <li><b>移除单人</b>：本页点击「移除」按钮</li>
    <li><b>清空全部</b>：本页点击「清空全部」按钮</li>
  </ul>
</div>
<script>
function del(uid, name) {
  if (!confirm('确认移除「' + name + '」？')) return;
  fetch('http://rednot.manage/remove?user_id=' + encodeURIComponent(uid))
    .then(r => r.json()).then(d => { if (d.success) location.reload(); });
}
function clearAll(n) {
  if (!confirm('确认清空全部 ' + n + ' 条黑名单记录？\n此操作不可恢复')) return;
  fetch('http://rednot.manage/clear')
    .then(r => r.json()).then(d => { if (d.success) location.reload(); });
}
</script>
</body>
